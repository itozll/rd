...
proc ConfHit {domain} {
  ...
  set index 1
  foreach name [lsort -dictionary [array names SERVER_FROM_CONF]] {
    if {$domain == "" || [regexp "$domain" $name match]} {
...
proc ConfGateway {server} {
  ...
  lassign $server ip null null domain group
  set gateway [Conf_Gets gateway $ip $domain $group]
  ...
  # [gateway]
  #   pattern = ...
  return [Conf_Gateway_List $ip $domain $group]
}
...
proc Conf_Server_Init {line} {
  global SERVER_FROM_CONF
  set pat {^\s*(\S+)(?:\s+(\S+)(?:\s+(\S+))?)?}
  if {[regexp $pat $line match server alias group]} {
    if {$alias == ""} {
      set alias $server
      set key0 ""
    } else {
      set key0 "\033\[33;3;4m$alias\033\[0m|"
    }
    ...
    if {$group == ""} {
      ...
      set key1 "  \033\[31;1m$group\033\[0m"
    }

    set key "$key0$server$key1"

    if {$group == ""} {
      set group $alias
    }

    set username [eval Conf_Gets username $server $alias $group]
    if {$username == ""} {
      global CONF_USERNAME
    ...
    set password [Conf_Gets password $server $alias $group]
    if {$password == ""} {
      global CONF_PASSWORD
      set password $CONF_PASSWORD
    }

    set info "{$server} {$username} {$password} {$alias} {$group}"
